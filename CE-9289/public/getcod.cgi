...
function ssxl(xl_,r){var i_=new Image(1,1);i_.src="https://s17.shinystat.com/cgi-bin/csa.cgi?USER="+usHYUHS_+"&"+xl_+"&RM="+Math.round(Math.random()*2147483647);if(r){_ssrit(r);}else{_ssrit(500);}i_.onload=function(){_ssvoid();}}
...
function _ttr(_t){cCk("TTR_"+usHYUHS_,_t);} // for js reload insert this function before location.reload 
function _cttr(){ if (_tr=rCk("TTR_"+usHYUHS_)){document.cookie = "TTR_"+usHYUHS_+"=; expires=Thu, 01-Jan-70 00:00:01 GMT; path=/;";return("&RFS="+_tr);}else return("");}
...
function _sstepHYUHS(e){var oe=new Date();var _dt=Math.round((oe.getTime()-tbro_)/1000);if (_tr=_chkr())if ((_dt>=(_tr-2)) && (_dt <=(_tr+2))) _ttr(_tr);cCk("SN_"+usHYUHS_,stfCkHYUHS(snHYUHS_,svHYUHS_,_dt),2592000);}
function sseC(d){if (typeof(encodeURIComponent) == 'function') {return encodeURIComponent(d);} else {return escape(d);}}
function ssORDCK(){if (!okcvHYUHS_){if (typeof(_ssCONV) !== 'undefined') {okcvHYUHS_=_ssCONV;if ((typeof(_ssCurr) !== 'undefined') && (_ssCurr!='')) {return(_ssCONV+"&C_CUCY="+_ssCurr)}return(_ssCONV);}return("");}return(okcvHYUHS_);}
function ssORD(){var _par_="SSCONV="+ssORDCK();var argv = ssORD.arguments;if (_paguHYUHS.length>1) {_par_+="&"+_paguHYUHS}if (argv[0].length<=1) {_cidorHYUHS=""+o_.getDate()+o_.getMonth()+"-"+rssidHYUHS_;argv[0]=_cidorHYUHS;} else{ _cidorHYUHS=""+sseC(argv[0]);}for (var i = 0; i<argv.length && i<_cvt.length; i++) {if (argv[i]) _par_+="&"+_cvt[i]+sseC(argv[i]);}_sscAHYUHS[_idxcAHYUHS++] = _par_+pccHYUHS_+ssidHYUHS_;}
function ssORDnm() {var argv = ssORDnm.arguments;ssORD(argv[0], 0, 0, 0, argv[1], argv[2], argv[3]);}
function ssPROD(){ var _conv="SSCONV="+ssORDCK(); var _par_=_conv+"&"+_cvt[0]+_cidorHYUHS;var argv = ssPROD.arguments;for (var i = 0; i < argv.length && i<_cvp.length; i++) {if (argv[i]) _par_+="&"+_cvp[i]+sseC(argv[i]);}_par_+="&C_IXPR="+_idxcAHYUHS;_sscAHYUHS[_idxcAHYUHS++] = _par_;}
function ssPRODnm() {var argv = ssPRODnm.arguments;ssPROD(argv[0], argv[1], 0, argv[2], argv[3]);}
function ssCvTrack(){var _sscmp="";var _ssa="";_sscmp=rCk("SSIDC_"+usHYUHS_);_ssa=ssaRls2("SSA");for (var i = 0; i < _sscAHYUHS.length; i++) { if (_sscAHYUHS[i].indexOf("C_PRTO")>-1) { _sscAHYUHS[i]+="&C_NUPR="+(_sscAHYUHS.length-1); if (_sscmp) _sscAHYUHS[i]+="&CAMP="+_sscmp; } if (_ssa) _sscAHYUHS[i]+="&SSA="+_ssa; ssxlHYUHS(_sscAHYUHS[i]);}}
...
function scCk(cck,v,r){var ca=_fr_=_ioi_=_id_=_tuv_=_fhr_=_lhr_=_hr_="";var _lr_=r;var _v_=_p_=_pvc_=idx=1;var _tfr_=_tuv_=ud();if (!v) v=0;_hr_=_vhrHYUHS;if (self != top){try {_hr_=document.referrer;}catch(e_r){}}_lhr_=sseC(_hr_);if ((idx=_lr_.indexOf(document.domain))>0) {if (idx<9) _lr_="";}idx=_hr_.indexOf('?');if (idx>-1) _lhr_=sseC(_hr_.substr(idx));if (okcvHYUHS_) {_id_=rssidHYUHS_;}if (cck) {ca = cck.split('%G');_v_=parseInt(ca[0],10);if (!isNaN(ca[6])) {_pvc_=parseInt(ca[6],10)+1;}_p_=parseInt(ca[1],10)+1;if (!(parseInt(ca[7],10))) _tuv_=ud();else _tuv_=ca[7];if (otb_.getTime()-parseInt(v,10)>st_) {_v_++;_pvc_=1;_tuv_=o_.getTime();} _fr_=ca[2];_fhr_=ca[8];_tfr_=ca[3];if (!_lr_) _lr_=ca[4]; if (!_lhr_) _lhr_=ca[9];if (!ca[5]) ca[5]=_id_;  else _id_=ca[5];}else{_fr_=_lr_;_fhr_=_lhr_;}_lhr_="";_lr_="";_ioi_=""+_v_+"%G"+_p_+"%G"+_fr_+"%G"+_tfr_+"%G"+_lr_+"%G"+_id_+"%G"+_pvc_+"%G"+_tuv_+"%G"+_fhr_+"%G"+_lhr_; pccHYUHS_="&"+_cve[0]+_v_+"&"+_cve[1]+_p_+"&"+_cve[2]+_fr_+"&"+_cve[3]+_tfr_+"&"+_cve[4]+_lr_+"&"+_cve[5]+_id_+"&"+_cve[6]+_pvc_+"&"+_cve[7]+_tuv_;pccHYUHS_+="&"+_cve[8]+_lhr_+"&"+_cve[9]+_fhr_;return(_ioi_);}
function sswk_(tnow){var w = Math.floor( ( (tnow / 86400000 ) - 4) / 7);return w}
function stfCkHYUHS(ck,v,upt){var ca=_pt_=_iof_=can="";var _f_=uv_=uvw=1;var _tf_=ud();var _t_=_bu_=0;_ort=new Date();_ort.setTime(o_.getTime()+(1000*ssoffset_));if (!v) v=0;if (ck) {ca = ck.split('%G');_f_=parseInt(ca[2],10);_tf_=parseInt(ca[3],10);if (o_.getTime()-parseInt(v,10)>st_){_ot=new Date();_ot.setTime(parseInt(_tf_,10)+(1000*ssoffset_)); if(_ort.getUTCMonth()==_ot.getUTCMonth()){_f_++;}else{_f_=1;_tf_=ud();}}_t_=ca[0];_pt_=ca[1];_bu_=ca[4];if (ca[5]) can=ca[5];}if (upt) {if (_paguHYUHS.length>1) {_pt_=_paguHYUHS.substring(4);}else{_pt_=""+sseC(window.location.href);} _t_=upt;}_ot=new Date();_ot.setTime(parseInt(_bu_,10)+(1000*ssoffset_));if ((_ort.getUTCDay()==_ot.getUTCDay())&&(_ort.getUTCMonth()==_ot.getUTCMonth())&&(_ort.getUTCFullYear()==_ot.getUTCFullYear())) uv_=0;if (sswk_(_ort) == sswk_(_ot)) uvw=0;_iof_=""+_t_+"%G"+_pt_+"%G"+_f_+"%G"+_tf_+"%G"+ud()+"%G"+SScanHYUHS(1);tfHYUHS_="&TUP="+_t_+"&PTUP="+_pt_+"&FV="+_f_+"&UV="+uv_+"&US="+uvw+"&DUP="+(_bu_/1000)+"&PCNL="+can;return(_iof_);}
function _sslinkd(_uj,nodl){var pc,sqs,shr,a,b,c,d,h,exl;var sdl=document.location;if (!_uj || _uj=="") return;sqs = _uj.indexOf("?"); shr = _uj.indexOf("#"); a=rCk("SSC_"+usHYUHS_);b=rCk("SV_"+usHYUHS_,1);c=rCk("SN_"+usHYUHS_);h=a+b+c;d=_ssHash(h.toUpperCase());pc ="SSC__="+a+"&SV__="+b+"&SN__="+c+"&SHS__="+d;if (pc) {if (sqs==-1 && shr==-1) exl=_uj+"?"+pc;else if (shr==-1) exl=_uj+"&"+pc;else if (sqs==-1) exl=_uj.substring(0,shr-1)+"?"+pc+_uj.substring(shr);else exl=_uj.substring(0,shr-1)+"&"+pc+_uj.substring(shr);if (nodl) {return(exl);}else{ sdl.href=exl;}}else { if (nodl) {return(_uj);}else{ sdl.href=_uj;}}}
function _sspostd(_fj){var pc,sqs,shr,a,b,c,d,h;if (!_fj || !_fj.action) return;sqs = _fj.action.indexOf("?"); shr = _fj.action.indexOf("#"); a=rCk("SSC_"+usHYUHS_);b=rCk("SV_"+usHYUHS_);c=rCk("SN_"+usHYUHS_);h=a+b+c;d=_ssHash(h.toUpperCase());pc ="SSC__="+a+"&SV__="+b+"&SN__="+c+"&SHS__="+d;if (pc) {if (_fj.method = 'post') {var inp = document.createElement('input');inp.type = 'hidden';inp.name = '_sspostd';inp.value = pc;_fj.appendChild(inp);}else{if (sqs==-1 && shr==-1) _fj.action+="?"+pc;else if (shr==-1) _fj.action+="&"+pc;else if (sqs==-1) _fj.action=_fj.action.substring(0,shr-1)+"?"+pc+_fj.action.substring(shr);else _fj.action=_fj.action.substring(0,shr-1)+"&"+pc+_fj.action.substring(shr);}}return;}
...
function ckrld(){var dl=document.location;var qs=dla=a=b=c=d=e=h="";dla=dl.href.substring(dl.href.indexOf('#'));if (dla && dla!="") qs=dla+"&";qs+=dl.search.substring(1);if(qs && qs!="" && qs.indexOf("SV__=")>=0) {var pck=objQS(qs);if ( qs.indexOf("SSNP__=")>=0 ) {a=decodeURIComponent(pck['SSC__']);b=decodeURIComponent(pck['SV__']);c=decodeURIComponent(pck['SN__']);d=decodeURIComponent(pck['SHS__']);e=decodeURIComponent(pck['SSID__']);}else{a=pck['SSC__'];b=pck['SV__'];c=pck['SN__'];d=pck['SHS__'];e=pck['SSID__'];}h=a+b+c;if (_ssHash(h.toUpperCase()) != d) return;if (a && a!="") cCk("SSC_"+usHYUHS_,a,2592000);if (b && b!="") cCk("SV_"+usHYUHS_,b);if (e && e!="") cCk("SSID_"+usHYUHS_,e);if (c && c!="") cCk("SN_"+usHYUHS_,c,2592000);}}
function ssadbk(qs) { var img; img = new Image(); img.src = "//advm.brznetwork.com/commons/adsense.png"; img.onload = function() { return;}; img.onerror = function() { var iabk_=new Image(1,1); iabk_.src="https://s17.shinystat.com/cgi-bin/adbsa.cgi" + qs +"&RM="+Math.round(Math.random()*2147483647); iabk_.onload=function(){return;} }; }
function SSsdk(e){return("");} function SScanHYUHS(){return("");}var ssxlHYUHS=ssxl;var _cvt=new Array("C_IDOR=","C_PRTO=","C_PRTA=","C_PRSP=","C_NAZ=","C_PROV=","C_CITT=","C_USR0=","C_USR1=","C_USR2=","C_USR3=","C_USR4=","C_USR5=","C_USR6=","C_USR7=","C_USR8=","C_USR9=");var _cvp=new Array("C_IDPR=","C_QTA=","C_PRUN=","C_DEPR=","C_CATE=");
var _cidorHYUHS;usHYUHS_="TRO-primochefit";n_=c_="";l_=""+screen.width;y_=""+screen.height;v_=navigator;d_=document.referrer;dh_=window.location.href;var o_=new Date();var otb_=new Date();vu_="&VUT=-1";
ebl_="cdgnsdtomn`e";_edbl=tfHYUHS_=pccHYUHS_=okcvHYUHS_=_paguHYUHS=_vhrHYUHS=svHYUHS_=snHYUHS_=ckadHYUHS="";
var _cve=new Array("C_VISI=","C_PVIS=","C_REFP=","C_DATP=","C_REF=","C_IDCO=","C_PVCO=","C_DATV=","C_PAG=","C_PAGP=");ssidHYUHS_="&SSID=";par_="&NODW=yes&PAG=https://primochef.it/?refresh_ce";
var _sscAHYUHS= new Array();var _idxcAHYUHS=0;
tbro_=o_.getTime();o_.setTime(1000*1637343949);var ssoffset_=3600;var rssidHYUHS_=Math.round(Math.random()*ud());var st_=1800000;var csp_=par_.split('&');
for(_i__=0;_i__<csp_.length;_i__++) {if (csp_[_i__].indexOf("PAG=")>-1) _paguHYUHS=csp_[_i__];if (csp_[_i__].indexOf("SSCONV=")>-1) okcvHYUHS_=csp_[_i__].substring(7,csp_[_i__].length);if (csp_[_i__].indexOf("CKAD=")>-1) ckrld();}_edbl=sseXr(ebl_,1);r_=""+sseC(d_);
...
if (fd_>-1){cCk("SSIDC_"+usHYUHS_,dh_.substr(7+fd_,dh_.length),86400);}if(v_.appName!="Netscape"){c_=screen.colorDepth}else{c_=screen.pixelDepth}
if (svHYUHS_ = rCk("SV_"+usHYUHS_)){var det=otb_.getTime()-parseInt(svHYUHS_,10);vu_="&VUT="+det;if (det>st_) {cCk("SSID_"+usHYUHS_,rssidHYUHS_);ssidHYUHS_+=rssidHYUHS_;_vhrHYUHS=""+dh_;}else{ssidHYUHS_+=rCk("SSID_"+usHYUHS_);}}else {cCk("SSID_"+usHYUHS_,rssidHYUHS_);ssidHYUHS_+=rssidHYUHS_;_vhrHYUHS=""+dh_;}cCk("SV_"+usHYUHS_,udtb());
if (ssc_=rCk("SSC_"+usHYUHS_)){cCk("SSC_"+usHYUHS_,scCk(ssc_,svHYUHS_,r_),2592000);}else{cCk("SSC_"+usHYUHS_,scCk("",svHYUHS_,r_),2592000);}if (snHYUHS_=rCk("SN_"+usHYUHS_)){cCk("SN_"+usHYUHS_,stfCkHYUHS(snHYUHS_,svHYUHS_,""),2592000);}else{n_="&NUT=y";cCk("SN_"+usHYUHS_,stfCkHYUHS("",svHYUHS_,""),2592000);}
var ssqS_="https://s17.shinystat.com/cgi-bin/csa.cgi?USER="+usHYUHS_+par_+"&REFER="+r_+"&COLOR="+c_+"&SIZE="+l_+"&RES="+l_+"X"+y_+k_+hr_+j_+vu_+n_+ssidHYUHS_+tfHYUHS_+_cttr()+SSsdk(svHYUHS_)+SScanHYUHS()+"&JS=Y&VJS=4035CSA";
...
if(1) {
    ...
    function ssaif(){        
        ...
        var ssals_="";
        if (sls_) {ssals_=sls_;}
        var uri="https://codicebusiness.shinystat.com/cgi-bin/getcod.cgi?IFSSA=yes&AFF=1&IDS=18290022&SSA="+ssals_+"&RF="+rf_+"&HR="+ssaE(window.location.href)+ssaCss()+"&NV="+nvs+"&RM="+Math.round(Math.random()*2147483647); 
...
function check_custom_consent(nck,value,log){
    ...
    if (infb!="0"){ ssaif();}
       ...
       var okgdpr=1;
       for(var i=0;i<nzgdpr.length;i++){ if (nzgdpr[i]=="MX") {okgdpr=0;break;}} 
       if (okgdpr){ssaif();}
    ...
    var _consentCallback = function(found) {
      ...
      if (found.length){
	      for (var i = 0; i < found.length; i++) { cmp+="$"+encodeURIComponent(found[i].name);}
              ...
              dcmp_.src="//s6.shinystat.com/cgi-bin/csa.cgi?USER=dcmp&PAG=18290022"+cmp+"&RM="+Math.round(Math.random()*2147483647);
...
if (1) {
  ...
  function affcan(){
    ...
    var aff_regexCanale = new Array('');      

    var aff_not_regexCanale = new Array('');
    var aff_idxCanale = new Array('');
    var aff_cat = new Array('');
    var aff_tag = new Array('');  
    var aff_sis = new Array('');
  ...
  if (self == top && !idck) {
      ...
      var d_=new Date();
      var optout="0";
      d_.setTime((1000*1637343949)+(1000*3600));
      t_=d_.getTime();
      var l_=""+screen.width;
      var y_=""+screen.height;
      var si_=affrk("AFF_S_18290022");
      var n=affrk("AFF_18290022");
      var ug=affrk("AFF_UG_18290022");
      var uw=affrk("AFF_UW_18290022");
      var um=affrk("AFF_UM_18290022");
      var v=affrk("AFF_V_18290022");
      var faff="0";
      var ds=0;
      if (n) {
      ...
      if ((faff) && (!ds) && (11478 == 11478)){window.shbnr_stop_erog=1; window.postMessage('adc_sendevent|PASSBACK', '*');console.log("adc_sendevent|PASSBACK");}
     
       

      if (!parseInt(optout,10)) {
          affck("AFF_18290022",t_,2592000);
          affck("AFF_UG_18290022",t_,2592000);
          affck("AFF_UW_18290022",t_,2592000);
          affck("AFF_UM_18290022",t_,2592000);
          affck("AFF_V_18290022",t_);
      }

      if (!si_){
        si_=Math.round(Math.random()*t_);
              affck("AFF_S_18290022",si_);
      }
      ...
      if (ds) {
          ...
          if (!nd) {
              ...
              sftn.id="_sftn";
              sftn.async="async";
		if("TRO-primochefit"=="TRO-agrodolcepublichtmli" || "TRO-primochefit"=="TRO-wallstreetitaliapubl"){
			sftn.src=th_+"advs.brznetwork.com/cgi-bin/as.cgi?TEST=583&NWK_ID=3&PUB_ID=18290022&PUB_URL="+sseC(document.location.href)+"&PUB_REF="+r_+"&PUB_CAT="+catp+"&PUB_CAT2="+cats+"&TAGS="+srs[1]+"&SR="+l_+"X"+y_+affafl()+"&"+_jFkt()+"&SSID="+si_+"&OPTOUT="+optout+ssa+"&RND="+Math.round(Math.random()*2147483647);
		}
		else{
              		sftn.src=th_+"advs.brznetwork.com/cgi-bin/as.cgi?UP=0&NWK_ID=3&PUB_ID=18290022&PUB_URL="+sseC(document.location.href)+"&PUB_REF="+r_+"&PUB_CAT="+catp+"&PUB_CAT2="+cats+"&TAGS="+srs[1]+"&SR="+l_+"X"+y_+affafl()+"&"+_jFkt()+"&SSID="+si_+"&OPTOUT="+optout+ssa+"&RND="+Math.round(Math.random()*2147483647);
		}
               // Tm Check

               if ((11478 == 11478) || (11478 == 11394)){
		    ...
		    waitForTCFapi(function() {
		      ...
		      // in caso aspetto 1 secondo al max.
                ...
                checkTCFapiConsent();
                  ...
                  document.body.appendChild(sftn);
                  if ("18290022" == "18289824") { ssxl("SS_EVENT=ok%20ViewMax%20Caricato"); }
